...
def student_dashboard():
    ...
    # Get recent notifications (global table; schema has no user_id/user_type)
    notif_query = """SELECT * FROM notifications ORDER BY date DESC LIMIT 10"""
    notifications = execute_query(notif_query, fetch=True) or []
...
def claim_item():
    ...
    if request.method == 'POST':
        ...
        lost_item = execute_query(
            "SELECT lost_item_id, item_name FROM lost_items WHERE student_id = %s AND status = 'Unresolved' ORDER BY lost_date DESC LIMIT 1",
            (session['user_id'],),
        ...
        # Ensure the found item exists and is unclaimed
        found_item = execute_query("SELECT f_i_id, item_name, status FROM found_items WHERE f_i_id = %s", (found_id,), fetch=True, fetchone=True)
        if not found_item or found_item.get('status') != 'Unclaimed':
            return jsonify({'success': False, 'message': 'Selected found item is not available for claiming'})

        # Create match using stored procedure (keeps DB logic intact)
        # Stored procedure will INSERT into match_items and update statuses to 'Matched'
        call_procedure('MatchLostFound', (lost_item['lost_item_id'], found_id))

        # Get new match_id (the procedure inserts it)
        ...
        # IMPORTANT: schema requires proof_file NOT NULL -> if the user didn't upload a file, insert an empty blob (b'')
        if proof_file:
            proof_data = proof_file.read()
            if not proof_data:
                # avoid inserting NULL; use empty bytes
                proof_data = b''
        else:
            proof_data = b''
        ...
        # Notify staff that a claim was submitted (global notifications table)
    ...
    # GET: show unclaimed found items for claiming
...
def verify_claim(claim_id, action):
    ...
    # after_claim_update_notify trigger will insert the notification row automatically
...
def staff_match():
    ...
    lost_query = """
        SELECT li.lost_item_id, li.item_name, li.category, li.description,
               li.lost_date, li.lost_loc, li.status,
    ...
    found_query = """
        SELECT f.f_i_id, f.item_name, f.category, f.description,
               f.found_date, f.found_loc, f.status,
               CASE WHEN f.report_student_id IS NOT NULL THEN s.name ELSE st.name END AS reporter_name
        FROM found_items f
        LEFT JOIN student s ON f.report_student_id = s.student_id
        LEFT JOIN staff st ON f.report_staff_id = st.staff_id
    ...
    for lost in lost_items:
        ...
        for found in found_items:
            ...
            if score > 0.7 and same_category and date_diff <= 7:
                ...
                notif_msg = f"Potential match found: Lost '{lost['item_name']}' ↔ Found '{found['item_name']}'"
                create_notification(notif_msg)
...
def get_notifications():
    """Get notifications for current user (global notifications table)"""
    if 'user_id' not in session:
        return jsonify({'success': False})
    
    # Your notifications table is global (no user_id/user_type columns), so return latest notifications
    query = """SELECT * FROM notifications ORDER BY date DESC LIMIT 20"""
    notifications = execute_query(query, fetch=True) or []
